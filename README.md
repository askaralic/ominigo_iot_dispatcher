...
## Configuration
- Update `TelemetryDBBridgeService/appsettings.json` with your values:
...
## Run locally
1) `cd TelemetryDBBridgeService`
2) `dotnet restore`
3) `dotnet run`

## Publish + install as Windows service
1) Publish: `dotnet publish -r win-x64 -c Release --self-contained false`
2) Install: `sc create OminigoIoTDispatcher binPath= "<publish_path>\\TelemetryDBBridgeService.exe" start= auto`
